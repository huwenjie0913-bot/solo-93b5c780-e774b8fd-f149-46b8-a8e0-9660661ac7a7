...
求解器为每个“吸入/混匀前枪头边界”建立单位容量顶点，其余液体转移为无限容量边，将所有首次到达目标孔的污染节点（组分级与来源级）连接到超级汇点。由最大流-最小割定理，单位容量顶点割的大小就是阻断全部传播路径所需的最少换头或完全洗针次数；随后会重新模拟割点集合并验证阈值。两种情况明确返回 `infeasible` 且不给出动作：目标孔初始液体本身就含非允许来源；或污染来自本步直接吸入并排到目标孔（新吸入液体不经过可割的枪头边界，任何换头/洗针都无法阻止）。任何最终建议都必须经重新模拟确认能消除全部违规，否则按无可行方案处理。

版本比较按 A、B 两版来源规则的并集生成 `source_comparison`（规则只存在于某一版时，另一侧判定为缺失），总体结论 `b_improves_all_failed_targets` 基于两版真实来源审查结果：A 失败的目标在 B 全部通过且 B 未引入新的组分级或来源级失败才为真。
